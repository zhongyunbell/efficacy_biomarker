...
def linear_dataset(beta, num_common_causes, num_samples, num_instruments=0,
                   ...
                   w_multiplier = 1, x_multiplier = 1): # RH: changed added x_multiplier, w_multiplier
    ...
    if type(beta) not in [list, np.ndarray]:
        beta_Tx = np.repeat(beta, num_treatments)
    ...
    if num_common_causes > 0:
        range_c1 = max(beta_Tx)*0.5
        range_c2 = max(beta_Tx)*0.5
        ...
        c1 = np.random.uniform(range_c1/2, range_c1, (W_with_dummy.shape[1], num_treatments))
        c2 = np.random.uniform(range_c2/2, range_c2, W_with_dummy.shape[1])
    ...
    if num_effect_modifiers >0:
        range_ce = np.repeat(beta, num_effect_modifiers)*0.5
...
#         import pdb; pdb.set_trace()
        ce = np.random.uniform(range_ce*x_multiplier/2, range_ce*x_multiplier, X_with_categorical.shape[1])
    ...
    print(f'Coefficients V:{c1}, W:{c2*w_multiplier}, X:{ce*x_multiplier}')

    def _compute_y(t, W, X, FD, beta_Tx, c2, ce, cfd2):
        ...
        if num_frontdoor_variables > 0:
            ...
            y += t @ beta_Tx
        if num_common_causes > 0:
            y += W @ c2*w_multiplier # RH: changed added w_multiplier
        if num_effect_modifiers > 0:
            y += (X @ ce*x_multiplier) #+ np.prod(t, axis=1) # RH: changed added x_multiplier and * to +
        if outcome_is_binary:
            y = np.vectorize(stochastically_convert_to_binary)(y)
        return y

#     import pdb; pdb.set_trace()
    y = _compute_y(t, W_with_dummy, X_with_categorical, FD, beta_Tx, c2, ce, cfd2)
    ...
    ate = np.mean(
            _compute_y(T1, W_with_dummy, X_with_categorical, FD_T1, beta_Tx, c2, ce, cfd2) -
            _compute_y(T0, W_with_dummy, X_with_categorical, FD_T0, beta_Tx, c2, ce, cfd2))
