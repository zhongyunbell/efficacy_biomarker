...
from .shap_based_analysis import xgb_shap, optimize_xgb, objective_model, SHAP_CV
import shap
...
def SHAP_based_FE(df, target, orderedFeatures_initial, n_steps, xgb_hyperparams, max_evals=0, verbose=True):
    ...
    for f,n_feats in tqdm.notebook.tqdm(enumerate(n_features), total = len(n_features), disable=False, desc='SHAP FE'):
        ...
        curr_r2 = curr_step.model_performance(verbose=verbose)
        all_r2s.append(curr_r2)
        if f!=0:
            features_removed.append(all_features[n_features2remove[f-1]:])
            all_features= all_features[:n_features2remove[f-1]]
    
    mean_r2 = np.array(all_r2s).mean(axis=1)
    optimal_features = orderedFeatures_initial[:n_features[np.where(mean_r2==np.max(mean_r2))[0][0]]]
    
    return dict(FE_method = FE_method,
                mean_r2 = mean_r2,
                std_r2 = np.array(all_r2s).std(axis=1),
                n_features = n_features, 
                features_removed = features_removed,
                optimal_features = optimal_features)

def SHAP_based_RFECV(df, target, orderedFeatures_initial, xgb_hyperparams=None, n_steps=25, max_evals=0,verbose=True):
    """
    ...
    for f,n_feats in tqdm.notebook.tqdm(enumerate(n_features), total = len(n_features), disable=False, desc='SHAP RFECV'):    
        ...
        curr_step = xgb_shap(df = pd.concat([df[remaining_features], df[target]],axis=1),
            target=target, max_evals=max_evals, n_folds_SHAP=3, verbose=verbose, xgb_hyperparams=xgb_hyperparams)
        if max_evals > 0:
            if verbose: 
                print("Step:", f, "n_feats", n_feats, "Tuning current...")
    ...
    mean_r2 = np.array(all_r2s).mean(axis=1)
    optimal_features = ordered_features_posthoc[:n_features[np.where(mean_r2==np.max(mean_r2))[0][0]]]
    ...
    for f,n_feats in tqdm.notebook.tqdm(enumerate(n_features), total = len(n_features), disable=False, desc='Boruta FE'):    
        ...
        if max_evals > 0:
            if verbose: 
                print("Step:", f, "n_feats", n_feats, "Tuning current...")
            curr_step.tune_model(verbose=False) 
        curr_r2 = curr_step.model_performance(verbose=False)
        all_r2s.append(curr_r2)
    mean_r2 = np.array(all_r2s).mean(axis=1)
    optimal_features = orderedFeatures_initial[:n_features[np.where(mean_r2==np.max(mean_r2))[0][0]]]
...
class feature_elimination(analysis):
    def __init__(self, df, target, max_evals=25, xgb_hyperparams=None, keep_features=[], verbose=True, remove_outliers=False, outputs_dir='./'): 
        analysis.__init__(self, df=df, target=target, remove_outliers=remove_outliers)
    ...
    def tune_model(self, verbose=True):
        ...
        xgb_hyperparams.update({"objective": "reg:squarederror", 'tree_method':"exact", "eval_metric":"error"})
        end=time.time()
        if verbose:
    ...
    # SHAP analysis
    @LazyProperty
    def shap_values_raw(self):
        SHAP_outputs = SHAP_CV(df=self.df, mdlFeatures=self.mdlFeatures, target=self.target, xgb_hyperparams=self.xgb_hyperparams, nFolds=self.n_folds_SHAP, verbose=self.verbose)
        setattr(self, 'R2_CV_SHAP', SHAP_outputs['CV_R2'])
        setattr(self, 'predictionsCV_SHAP', SHAP_outputs['valPredictions_vec'])
        setattr(self, 'SHAP_outputs', SHAP_outputs)
        setattr(self, 'meanExpValue', SHAP_outputs['meanExpRaw'])
        return SHAP_outputs['shap_values_raw']

    @LazyProperty
    def shapDF_raw(self):
        shapDF_raw = pd.DataFrame(self.shap_values_raw, columns=self.mdlFeatures)
        shapDF_raw['meanExpValue'] = self.meanExpValue
        return shapDF_raw
    ...
    def orderedFeatures_initial(self):
        ...
        if self.xgb_hyperparams == None: 
            self.tune_model()
        vals= np.abs(self.shap_values_raw).mean(0) # Ordered by average |SHAP value| 
    ...
    def initial_shap_summary_plots(self, selectionVec = None, show=True, max_display=30, figsize=(40,12), save_fig=False, outputs_dir=None): 
        ...
        shap.summary_plot(self.shap_values_raw[selectionVec,:], features=self.df.loc[selectionVec,:].drop(columns=self.target), max_display=max_display, show=False, plot_size=(figsize[0]/3,figsize[1]), plot_type='bar')
        #plt.subplot(1,2,2)
        fig.add_subplot(gs[0, 1:])
        shap.summary_plot(self.shap_values_raw[selectionVec,:], features=self.df.loc[selectionVec,:].drop(columns=self.target), max_display=max_display, show=False, plot_size=(2*figsize[0]/3,figsize[1]))
        plt.tight_layout()
        if selected == 'all':
            fig.text(.1, 1, "R2 (CV): {:.3f}".format(self.R2_CV_SHAP), ha='left', fontsize = 18)
        if save_fig:
            if outputs_dir==None: 
    ...
    def run_SHAP_based_RFECV(self, n_steps=30, max_evals=0, verbose=False):
    ...
    def plot_FE_comparison(self, save_fig=False, return_fig=False, outputs_dir=None):
        ...
        if len(all_FE_outputs)>0:
            ...
            for t,out in enumerate(all_FE_outputs): 
                curr_plot = ax.plot(out['n_features'], out['mean_r2'], '-o', label = out['FE_method'])
                color= curr_plot[0].get_color()
                ax.fill_between(out['n_features'], out['mean_r2']-out['std_r2'], out['mean_r2']+out['std_r2'], alpha = .2, label=out['FE_method'])  
                # Optimal lines:
                ax.vlines(x=len(out['optimal_features'])- .1*t, ymin =.2 , ymax=np.mean(out['mean_r2'] - 1.5*out['std_r2']), linestyles='--', lw = 3, color=color)
                for idx, x in enumerate(out['n_features']):
                    if idx%2==1:
                        ax.text(x, out['mean_r2'][idx] - .4*t*out['std_r2'][idx], "{:.3f}±{:.2f}".format(out['mean_r2'][idx], out['std_r2'][idx]), 
    ...
    def get_union_features(self, importance_sort=True):
        ...
        # Don't save union_features as an attribute unless it has been ordered first
    ...
    def run_forward_selection(self, features=None, verbose=True, n_folds=5, n_reps=5):
        ...
        sem_scores = []
        # For loop
        for f in tqdm.notebook.tqdm(range(len(init_list)), disable=False, desc='Forward selection'): # defines the initial order we'll do the step_wise forward selection
        ...
        df_d = dict(step = steps, mean_r2 = mean_scores,
                    std_r2 = std_scores, sem_r2 = sem_scores,
    ...
    def plot_forward_selection(self, save_fig=False, return_fig=False, outputs_dir=None, figsize=(20,8)):
        ...
        ax.plot(np.array(self.forward_selection_results.index)+1, 
                            out['mean_r2'], '-o', color='gray', alpha=.2, label = "forward selection")

        ax.fill_between(np.array(self.forward_selection_results.index)+1, 
                        out['mean_r2']-out['std_r2'], 
                        out['mean_r2']+out['std_r2'], color='gray', alpha = .2, label='STD')  
            
        for idx, x in enumerate(self.forward_selection_results.index+1):
            ax.text(x, out['mean_r2'][idx] - 1.1* out['std_r2'][idx], "{} {:.3f}±{:.2f}".format(out['step'][idx], out['mean_r2'][idx], out['std_r2'][idx]), 
        ...
        for curr_outcome, color in zip(['Included', 'Tentatively included', 'Excluded'],['g', 'yellow', 'r']):
            ...
            mean_r2s = [out['mean_r2'][idx] for idx,outcome in enumerate(out['outcome']) if (outcome==curr_outcome) ]
            ax.scatter(x=idxs, y=mean_r2s, color=color, label = curr_outcome, s=100)
        ...
        if type(verbose) == type(None):
            ...
            try:
                ...
                for model in models:
                    ...
                    mean_r2s.append(mean_score)
                    std_r2s.append(std_score)
    ...
    def plot_model_comparison(self, outputs_dir=None, return_fig=False, save_fig=False, figsize=(6,4)):
        ...
        for idx,model in enumerate(performance_comparison_outputs['models']):
            label = model + "\n{:.3f}±{:.2f}".format(performance_comparison_outputs['mean_r2s'][idx], performance_comparison_outputs['std_r2s'][idx])
            plt.errorbar(x=idx, y=performance_comparison_outputs['mean_r2s'][idx], 
                        yerr=performance_comparison_outputs['std_r2s'][idx], 
                        label=label, fmt='ok')
            labels.append(label)
        ...
        ylims = (np.mean(performance_comparison_outputs['mean_r2s']) - 2* np.mean(performance_comparison_outputs['std_r2s']),
            np.mean(performance_comparison_outputs['mean_r2s']) + 2* np.mean(performance_comparison_outputs['std_r2s']))
