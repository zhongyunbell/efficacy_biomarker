...
section = st.radio('Section:',
         ...
         'SHAP-based exposure interactions',
         'Cut-off analysis', 
...
if section == 'Dataset':
    ...
    st.write(msno_fig)
    data_container = st.expander(label="Optimized dataset")
    with data_container: 
...
elif section == 'SHAP-based exposure interactions': 
    
    selectionVec=None
    st.write(analysis.shap_exposure_impacts(selectionVec=selectionVec, return_fig=True))
    st.write(analysis.shap_interaction_summary(feature=exposure_var, selectionVec=selectionVec, show=False))
    fig = analysis.shap_interaction_summary(feature=exposure_var, selectionVec=selectionVec, show=False, max_display=len(analysis.mdlFeatures))
    interaction_features = list(reversed([s._text for s in fig.get_children()[1].get_yticklabels()]))
    import seaborn as sns
    import matplotlib
    col1, col2=st.columns(2)
    with col1:
        features = st.multiselect(f'show feature(s) interactions with {exposure_var}', interaction_features, default=interaction_features[1])
        feature=features[0]
        feature_thr = st.slider(min_value=float(analysis.df[feature].min()), max_value=float(analysis.df[feature].max()), label=f'potential cutoff ({feature})')
        s=st.slider('marker size', min_value=1, max_value=100, value=10)
    with col2:
        color_by = st.selectbox(f'color by', interaction_features, index=interaction_features.index(exposure_var))
        exposure_range = st.slider(min_value=float(analysis.df[exposure_var].min()), max_value=analysis.df[exposure_var].max(), value=(float(analysis.df[exposure_var].min()), float(analysis.df[exposure_var].max())), label=f'min/max {exposure_var}')
        if color_by != exposure_var:
            ...
            selectionVec=(analysis.df[exposure_var]>=exposure_range[0]) & (analysis.df[exposure_var]<=exposure_range[1]) & (analysis.df[color_by]>=feature_range[0]) & (analysis.df[color_by]<=feature_range[1])
        else:
            selectionVec=(analysis.df[exposure_var]>=exposure_range[0]) & (analysis.df[exposure_var]<=exposure_range[1])
        color_norm = st.selectbox('color norm', options=['linear', 'log'])
    # st.write(shap_interaction_plot(analysis, feature, exposure_thr_vals=exposure_thr_vals, feature_thr=feature_thr, exposure_var=exposure_var, selectionVec=selectionVec, return_fig=True))
    
    st.write(shap_interaction_plot_combined(analysis, exposure_var=exposure_var, features=features, hue =color_by, selectionVec=selectionVec, return_fig=True, s=s, feature_thr=feature_thr, color_norm=color_norm))
    ...
    def generate_interaction_chart(analysis, feature, exposure_var):
        ...
        fig1 = figure(tools=['crosshair', 'lasso_select'], 
                 plot_width=500, plot_height=500, title=f'ER relationship colored by {feature}',
                  ...
                  x_range=[tmpDF[exposure_var+"_value"].quantile(.01)-1*tmpDF[exposure_var+"_value"].std(),
                           tmpDF[exposure_var+"_value"].quantile(.99)+2*tmpDF[exposure_var+"_value"].std()], 
        ...
        fig2 = figure(tools=['crosshair', 'lasso_select'], 
                    plot_width=500, plot_height=500, title=f'{feature}-{exposure_var} interaction effect colored by {exposure_var}',
                      ...
                      tmpDF['interaction_effect'].quantile(.99)+2*tmpDF['interaction_effect'].std()])
        ...
        fig3 = figure(tools=['crosshair', 'lasso_select'], 
            plot_width=500, plot_height=500, title=f'{feature}-{exposure_var} interaction effect colored by {feature}',
              ...
              y_range=[tmpDF['interaction_effect'].quantile(.01)-2*tmpDF['interaction_effect'].std(),tmpDF['interaction_effect'].quantile(.99)+2*tmpDF['interaction_effect'].std()])
    ...
    if True: # This is applicable for both dataset types
        ...
        if dataset_type == 'Synthetic data':
            ...
            def st_generate_synthetic_binary_classification_df(df, target, mdl_vars, oversampler_name=oversampler_name, desired_rows=2000, verbose=False, random_state=0):
                ...
                return generate_synthetic_binary_classification_df(df, target, oversampler_name=oversampler_name, desired_rows=desired_rows, verbose=False, random_state=random_state, discrete_columns=True)
